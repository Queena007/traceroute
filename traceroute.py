        ...
        for tries in range(TRIES):
            ...
            try:
                ...
                if whatReady[0] == []: # Timeout
                    ...
                    df.append({'hop':ttl, 'try':tries, 'IP':destAddr, 'hostname':hostname, 'timeout': TIMEOUT}, ignore_index=True)
                    print (df)
                    #Fill in end
                ...
                try: #try to fetch the hostname of the router that returned the packet - don't confuse with the hostname that you are tracing
                    ...
                    print(request_type, code, checksum, packetID, sequence)
                    #Fill in end
                ...
                if types == 11:
                    ...
                    df.append({'hop':ttl, 'try':tries, 'IP':destAddr, 'hostname':hostname, 'timeout': 'Error'}, ignore_index=True)
                    #Fill in end
                break
    return df
